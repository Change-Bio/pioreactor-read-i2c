...
class I2CToVoltage(LongRunningBackgroundJob):
    ...
    def __init__(self, unit: str, experiment: str, **kwargs):
        ...
        # Parse comma-separated list of I2C addresses
        i2c_addr_str = config.get("i2c_to_voltage.config", "i2c_addr")
        self.i2c_addresses = [int(addr.strip(), 16) for addr in i2c_addr_str.split(",")]
        ...
        # Initialize buffers for each ADC address and channel combination
    ...
    def on_ready(self):
        addr_list = ", ".join([f"0x{addr:02X}" for addr in self.i2c_addresses])
        self.logger.info(
            f"I2C to Voltage started: bus={self.i2c_bus}, addresses=[{addr_list}], "
    ...
    def on_disconnected(self):
        addr_list = ", ".join([f"0x{addr:02X}" for addr in self.i2c_addresses])
        self.logger.debug(f"Disconnecting from I2C addresses: {addr_list}")

    def read_single_ended(self, i2c_addr: int, channel: int) -> Tuple[int, float]:
        """Read a single-ended channel (AINx vs GND) from a specific I2C address."""
        assert 0 <= channel <= 3
        ...
        with SMBus(self.i2c_bus) as bus:
            bus.write_i2c_block_data(
                i2c_addr, CONF, [(conf >> 8) & 0xFF, conf & 0xFF]
            )
            time.sleep(0.001)  # allow conversion (~0.6 ms needed, reduced for faster sampling)
            hi, lo = bus.read_i2c_block_data(i2c_addr, CONV, 2)
    ...
    def sample_all_channels(self):
        ...
        try:
            for i2c_addr in self.i2c_addresses:
                for channel in range(4):
                    raw, volts = self.read_single_ended(i2c_addr, channel)
                    self.channel_buffers[i2c_addr][channel].append(volts)
                    ...
                    if len(self.channel_buffers[i2c_addr][channel]) > max_buffer_size:
    ...
    def publish_filtered_voltages(self):
        ...
        try:
            for i2c_addr in self.i2c_addresses:
                for channel in range(4):
                    if len(self.channel_buffers[i2c_addr][channel]) == 0:
                        continue

                    if self.enable_filtering and len(self.channel_buffers[i2c_addr][channel]) >= self.moving_avg_window:
                        # Apply moving average filter on last N samples
                        window = self.channel_buffers[i2c_addr][channel][-self.moving_avg_window:]
                        filtered_voltage = sum(window) / len(window)
                    else:
                        # No filtering or not enough samples - use latest reading
                        filtered_voltage = self.channel_buffers[i2c_addr][channel][-1]

                    # Publish with address in topic name
                    topic_suffix = f"0x{i2c_addr:02X}_A{channel}"
                    self.publish(
    ...
    def run_noise_diagnostics(self):
        ...
        try:
            ...
            # Collect samples for each ADC address and channel
            for i2c_addr in self.i2c_addresses:
                for channel in range(4):
                    diagnostic_samples = []
                    ...
                    topic_suffix = f"0x{i2c_addr:02X}_channel_{channel}"
                    self.publish(
                        f"pioreactor/{self.unit}/{self.experiment}/i2c_to_voltage/diagnostics/{topic_suffix}",
                        json.dumps(diagnostic_results),
                    )

                    self.logger.info(
                        f"0x{i2c_addr:02X} Channel {channel} diagnostics: mean={mean_voltage:.4f}V, "
